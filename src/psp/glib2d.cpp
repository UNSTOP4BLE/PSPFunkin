...
void g2dAdd()
{
    ...
    OBJ.rot_x = OBJ.x + (OBJ.scale_w / 2);
    OBJ.rot_y = OBJ.y + (OBJ.scale_h / 2);
...
void DrawZoomG2DTex(g2dTexture* tex, Rect *Img, FRect *Disp, bool linear, float angle, int alpha, float zoom)
{
    ...
    float x = (Disp->x - G2D_SCR_W/2) * zoom + G2D_SCR_W/2;
    float y = (Disp->y - G2D_SCR_H/2) * zoom + G2D_SCR_H/2;
    float w = Disp->w * zoom;
    float h = Disp->h * zoom;
    if (x+w >= 0 && x <= G2D_SCR_W && y+h >= 0 && y <= G2D_SCR_H)
    {
        g2dBeginRects(tex);
        g2dSetCropXY(Img->x, Img->y);
        g2dSetCropWH(Img->w, Img->h);
